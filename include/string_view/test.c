...
void sv_assert_eq_int64_t(const char *file, size_t line,
                           const char *expected_expr, int64_t expected,
                           const char *actual_expr, int64_t actual)
{
    if (expected != actual) {
        fprintf(stderr, "%s:%zu: FAILED: %s == %s\n",
                file, line, expected_expr, actual_expr);
...
int main(void)
{
    ...
    { // Chop by delimeters (many)
        // Existing
        {
            String_View input = SV_STATIC("hello = world");
            String_View res = sv_chop_by_delims(&input, " =");
            ASSERT_EQ(String_View, SV("hello"), res);
            ASSERT_EQ(String_View, SV("= world"), input);
        }
        ...
        // Non-Existing
        {
            ...
            String_View res = sv_chop_by_delims(&input, "!>/");
            ASSERT_EQ(String_View, SV(""), input);
            ASSERT_EQ(String_View, SV("hello = world"), res);
        }
    }
    ...
    {
        {
            ...
            ASSERT_EQ(int64_t, 1234567890, sv_to_i64(input));
            ASSERT_EQ(String_View, input, SV("1234567890"));
        }
        {
            String_View input = SV_STATIC("-123456789");
            ASSERT_EQ(int64_t, -123456789, sv_to_i64(input));
        }
    }
