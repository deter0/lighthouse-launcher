...
bool find_icon_helper(String icon_name, int icon_size, String theme, char *icon_path, size_t icon_path_max) {
  bool status = lookup_icon(icon_name, icon_size, theme, icon_path, icon_path_max);
  if (status == true) {
    return true;
  }
...
int main() {
  ...
  assert(find_icon("firefox", 32, NULL, buffer, sizeof(buffer)) == true);
  assert(strcmp(buffer, "/usr/share/icons/hicolor/32x32/apps/firefox.png") == 0);
  assert(find_icon("firefox", 0, NULL, buffer, sizeof(buffer)) == true);
